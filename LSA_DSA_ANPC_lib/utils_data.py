...
#         layer_names = ["5/relu6"]
        layer_names = ['0/relu', '1/layer1/0/relu', '2/layer1/1/relu', '3/layer1/2/relu', '4/layer2/0/relu', '5/layer2/1/relu', '6/layer2/2/relu', '7/layer2/3/relu', '8/layer3/0/relu', '9/layer3/1/relu', '10/layer3/2/relu', '11/layer3/3/relu', '12/layer3/4/relu', '13/layer3/5/relu', '14/layer4/0/relu', '15/layer4/1/relu', '16/layer4/2/relu']
        num_layer = 17
...
#         cluster_num = 7
    ...
    elif dataset == "svhn" or dataset == "alexnet" or dataset == "SVHN":
        sample_threshold = 0.8
        cluster_threshold = 0.8
        cluster_num = 3
    else :
    ...
    return sample_threshold, cluster_threshold
...
def get_dataset(dataset):
    ...
    if  dataset == "mnist":
        ...
        dataset_train = vdt.MNIST(root='./deephunter/models/data/',transform=test_transform, train=True)
        x_train, y_train = dataset_train.data, dataset_train.targets
        # x_train = x_train.transpose(0,2,3,1)
        # x_train = x_train # - (1.0 - CLIP_MAX)
        # x_train = x_train.astype("float32")

    elif  "cifar10" in  dataset or "cifar10conv" in  dataset or "cifar10vgg" in  dataset:
        ...
        import torchvision.transforms as vdf
        test_transform = vdf.ToTensor()
        dataset_train = vdt.CIFAR10(root='./deephunter/models/data/',transform=test_transform, train=True)
        x_train, y_train = dataset_train.data, dataset_train.targets
        # x_train = x_train.transpose(0,2,3,1)
        # x_train = x_train # - (1.0 - CLIP_MAX)
        # x_train = x_train.astype("float32")

    elif  "SVHN" in dataset or "alexnet" in dataset or "svhn" in dataset:
        import torchvision.datasets as vdt
        import torchvision.transforms as vdf
        test_transform = vdf.ToTensor()
        dataset_train = vdt.SVHN(root='./deephunter/models/data/',transform=test_transform, split="train")
    ...
    if test_dataset is None:
        ...
        test_dataset = torch.utils.data.TensorDataset(torch.FloatTensor(x_train), torch.LongTensor(y_train))
