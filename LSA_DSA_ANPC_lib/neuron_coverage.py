...
class Coverager():
    def __init__(self, model, arc, threshold, num_classes=10, num_cluster=10):
        ...
        for cla in range(num_classes):
            ...
            root_path = "./cluster_paths/{}/trains/".format(arc)
            picked_samples_fname = root_path + "threshold{}_class{}_paths.pkl".format(threshold, cla)
            # picked_samples_fname = "cluster_paths/{}/num_cluster{}_threshold{}_class{}_cluster{}_paths.pkl".format(arc, num_cluster, threshold, cla, clu)
            # 是训练集的路径
    ...
    def random_dataset(self, num):
        return np.random.choice(self.class_num, num)
    ...
    def get_rand_dg(self, dnn):
        ...
        for i in range(self.class_num):
            abst_path = []
            for j in range(self.total_cluster):
                abst_path.append(self.get_rand_path(dnn))
...
#                 print("dis", dis)
    ...
    def Intra_NPC(self, X, bucket_m, threshold, mode="mix", simi_soft=False, useOldPaths_X=False, arc=""):
        total = self.class_num * bucket_m
        covered = set()
        unit = 1/bucket_m
...
#                 print(dis)
    ...
    def Layer_Intra_NPC(self, X, bucket_m, threshold, mode="mix", simi_soft=False, useOldPaths_X=False, arc=""):
        total = self.class_num  * bucket_m * self.layer_num
        covered = set()
        unit = 1/bucket_m
...
if __name__ == '__main__':
    ...
    dnn = Coverager.random_nn()
    #generate the decision graph with n abstract paths
    dg = Coverager.get_rand_dg(dnn)
    #generate test suite
    X = Coverager.random_dataset(2000)
    bucket = 100
    print("Intra-P-NPC", Coverager.Intra_NPC(dnn, dg, X, bucket))
    print("Intra-L-NPC", Coverager.Layer_Intra_NPC(dnn, dg, X, bucket))
