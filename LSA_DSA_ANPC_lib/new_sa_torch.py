...
def _get_train_target_ats(model, x_train, x_target, target_name, layer_names, args):
    ...
    #     # print(infog("Found saved test {} ATs, skip serving").format(target_name))
...
#     print (type(train_ats),train_ats.shape)
    ...
    kdes, removed_cols = _get_kdes(train_ats, train_pred, class_matrix, args, paths=paths,
                                   path_layer=path_layer, path=path)
...
def fetch_newMetric(model, ori_model, x_train, x_target, y_test, sample_threshold, target_name, layer_names, 
                    args, cluster_paths, path_layer, samples_clusters, fetch_func= lambda x:x[0], fakePath=False, rest=False):
    
    saved_target_path = _get_saved_path(
        args.save_path, args.dataset, args.arch, target_name, layer_names, fileid=None,
    )
    if os.path.exists(saved_target_path[2]):
        print(infog("Found saved test {} paths, skip serving").format(target_name))
        # In case target_ats is stored in a disk
...
#                 )
        ...
        dataloader = Data.DataLoader(dataset=x_target, batch_size=64, shuffle=False)
        #####################
        for step, x in enumerate(dataloader):  # 提取每个输入的path
...
#   print("getting distance......")
    for i in tqdm(range(len(path_x))):
        p = path_x[i]
        cla = target_pred[i]   # 预测为哪一类
        clu, best_sim = find_closed_cluster(p, cluster_paths, cla)   # 最近的类的位置和距离
